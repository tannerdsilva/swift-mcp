...
func mcpApplicationToolID() {
    assertMCPExpansion(
        ...
        struct MyApp {
            ...
            private func _invokeTool(_ id: MyApp_ToolID, arguments: [String: Any], context: MCPContext) async throws -> MCPToolResult {
                switch id {
                    case .greet:
                        var tool = self.greet
            ...
            func toolID(named name: String) -> MyApp_ToolID? {
                if name == Greet.toolName {
                    return .greet
                }
                if name == Calculate.toolName {
                    return .calculate
                }
                return nil
            }
            ...
            func requiredAccess(named name: String) -> AccessLevel? {
                guard let id = toolID(named: name) else {
                    return nil
                }
                switch id {
                    case .greet:
                        return Greet.configuration.requiredAccess
                    case .calculate:
                        return Calculate.configuration.requiredAccess
                }
            }
            ...
            func toolCatalog(for callerAccessLevel: AccessLevel) -> [MCPToolDescriptor] {
                ...
                if callerAccessLevel >= Greet.configuration.requiredAccess {
                    catalog.append(MCPToolDescriptor(
                        name: Greet.toolName,
                        description: Greet.configuration.description,
                        parameters: Greet.discoverParameters()
                    ))
                }
                if callerAccessLevel >= Calculate.configuration.requiredAccess {
                    catalog.append(MCPToolDescriptor(
                        name: Calculate.toolName,
                        description: Calculate.configuration.description,
                        parameters: Calculate.discoverParameters()
                    ))
                }
                return catalog
            }
            ...
            static func main() async throws {
                let app = MyApp()
                let server = MCPServer(name: "test", version: "1.0.0", dispatcher: app)
                try await server.runService()
            }
        }

        extension MyApp: MCPToolDispatcher {
        }
        """,
    ...
    assertMCPExpansion(
        ...
        struct MyApp {
            ...
            func requiredAccess(named name: String) -> AccessLevel? {
                ...
                #if DEBUG
                        case .debug:
                            return DebugTool.configuration.requiredAccess
                        #endif
                }
            }
            ...
            func toolCatalog(for callerAccessLevel: AccessLevel) -> [MCPToolDescriptor] {
                ...
                #if DEBUG
                    if callerAccessLevel >= DebugTool.configuration.requiredAccess {
                        catalog.append(MCPToolDescriptor(
                            name: DebugTool.toolName,
                            description: DebugTool.configuration.description,
                            parameters: DebugTool.discoverParameters()
                        ))
                    }
                    #endif
                return catalog
...
func mcpApplicationWithAddress() {
    assertMCPExpansion(
        ...
        struct MyApp {
            ...
            static func main() async throws {
                let app = MyApp()
                let server = MCPServer(name: "test", version: "1.0.0", address: .localhostIPv4(port: 8080), dispatcher: app)
                try await server.runService()
            }
        }
...
func mcpApplicationWithTransport() {
    assertMCPExpansion(
        ...
        struct MyApp {
            ...
            static func main() async throws {
                let app = MyApp()
                let server = MCPServer(name: "test", version: "1.0.0", transport: MyCustomTransport(), dispatcher: app)
                try await server.runService()
            }
        }
