...
If a tool with the same name is already registered (in either registry), a
warning is logged and the existing tool is overwritten. Instance-registered
tools (``MCPServer/registerInstance(_:instance:)``) are access-filtered and
enforced exactly like type-registered ones, and are removed by
``MCPServer/unregister(_:)``. All registration APIs are safe to call while the
server is running.
